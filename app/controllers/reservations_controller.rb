...
class ReservationsController < ApplicationController
  ...
  before_action :set_ride, only: %i[new create complete accept reject]
  before_action :set_reservation, only: %i[show destroy accept reject show]
  ...
  def create # rubocop:disable Metrics/AbcSize
    if current_user.nil?
      create_account_and_sign_in if params[:user].present?
      return if @user.nil?
    end
    @reservation = Reservation.create!(reservation_params.merge(ride_id: @ride.id, user_id: current_user.id))
    ReservationMailer.reservation_passenger_request(@reservation).deliver_later if defined?(ReservationMailer)
  ...
  def complete
    ActiveRecord::Base.transaction do
      @ride.reservations.map { |reservation| reservation.update!(status: 'closed') }
  ...
  def mail_reservation_users(ride, method)
    ride.reservations.each do |reservation|
      ReservationMailer.send(method, reservation).deliver_later if defined?(ReservationMailer)
    end
  end
